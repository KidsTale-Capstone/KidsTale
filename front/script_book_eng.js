...
async function playTTS(pageContent, bookId, lang) {
    ...
    try {
        ...
        if (data.success) {
            ...
            isPlaying = true;
            audioButton.innerText = "Stop"; // 버튼을 '듣기 중지'로 변경

            // 오디오 재생이 끝났을 때 버튼을 다시 '듣기'로 변경
            currentAudio.onended = () => {
                isPlaying = false;
                audioButton.innerText = "Audio Book"; // 재생 완료 후 버튼 상태 변경
                currentAudio = null;
            };
...
function displayPage(pageIndex) {
    ...
    fetchBookData(ownerId, bookId, pageIndex, lang).then(pageData => {
        ...
        document.getElementById('book-author').innerText = `Author: ${pageData.author}`;
        ...
        audioButton.onclick = () => {
            if (isPlaying && currentAudio) {
                ...
                isPlaying = false;
                audioButton.innerText = "Audio Book"; // 버튼을 다시 '듣기'로 변경
            } else {
...
async function completeEdit() {
    ...
    try {
        const response = await fetch(`/book/update_page?id_book=${bookId}&page_index=${currentPage}&lang=eng`, {
            method: 'POST',
            headers: {
                'Authorization': `Bearer ${token}`
            },
